...
import static io.cloudslang.content.openstack.compute.entities.Inputs.Servers.SERVER_ID;
...
public class DeleteServer {
    /**
     * Deletes a server.
     * By default, the instance is going to be (hard) deleted immediately from the system, but you can set
     * reclaim_instance_interval > 0 to make the API soft delete the instance, so that the instance won’t be deleted
     * until the reclaim_instance_interval has expired since the instance was soft deleted. The instance marked as
     * SOFT_DELETED can be recovered via restore action before it’s really deleted from the system.
     * Preconditions:
     ...
     * The ports attached to the server, which Nova created during the server create process or when attaching interfaces
     * later, are deleted.
     ...
     * Troubleshooting:
     * If server status remains in deleting status or another error status, the request failed. Ensure that you meet the
     * preconditions. Then, investigate the compute back end.
     * The request returns the HTTP 409 response code when the server is locked even if you have correct permissions.
     * Ensure that you meet the preconditions then investigate the server status.
